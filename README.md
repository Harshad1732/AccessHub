...
This starts a `mcr.microsoft.com/mssql/server:2022-latest` container on port `1433`. Copy the connection string from [docker-compose.override.example.json](docker-compose.override.example.json) into `src/AccessHub.Api/appsettings.Development.json`.

> The data layer uses EF Core, so PostgreSQL is also possible by swapping the provider package and `UseSqlServer` call; SQL Server LocalDB is the default.
